...
export const printPDF = (id, x = 0, y = 0, width, height) => {
   ...
   printWindow.document.write(
      ` <html><head> <style>  td {border: 1px solid;}</style></head> <body>${printArea}</body></html>`
   );
   printWindow.focus();
   printWindow.print();
...
export const printPDFMultiple = async (className) => {
   ...
   for (let doc of printAreas) {
      let printWindow = window.open();
      let data = doc.innerHTML.toString();
      printWindow.document.write(
         ` <html><head> <style>  td {border: 1px solid;}</style></head> <body>${data}</body></html>`
      );
      printWindow.focus();
      printWindow.print();
   }
