...
export const getGraphStudentClassTotal = (schoolID) => {
   return (dispatch) => {
      ...
      return axios
         .get(`${url}/graph/class/total/${school}`)
         .then(({ data }) => {
...
export const getCurrentStudent = (admissionNumber) => {
   return (dispatch) => {
      ...
      try {
         return axios
            .get(`${url}/student/admissionNumber/${admissionNumber}`)
            .then(({ data }) => {
...
export const getStudentValidatedScoreRequest = (value) => {
   ...
   return (dispatch) => {
      ...
      try {
         return axios
            .get(
               `${url}/student/validate/${value.class}/${value.arm}/${value.subject}`
            )
            .then(({ data }) => {
...
export const getStudentClassAndArmRequest = (admissionNumber) => {
   return (dispatch) => {
      ...
      try {
         return axios
            .post(`${url}/student/classAndArm`, { admissionNumber })
            .then(({ data }) => {
               dispatch(getStudentClassAndArm(data));
            });
      } catch (error) {
         return dispatch(
            getStudentClassAndArm({ class: "No Class", arm: "No Arm" })
         );
      }
   };
};
