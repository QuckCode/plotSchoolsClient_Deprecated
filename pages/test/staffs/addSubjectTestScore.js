...
import { getStaffSubjects } from "../../../redux/actions/subject";
...
const TestAddPage = (props) => {
   ...
   const handleScoreSheetSubmit = () => {
      console.log({
         test: props.testBySubject.test,
         subject: props.testBySubject.subject,
      });
      axios
         .post(`${url}/student/subject/score/save`, {
            test: props.testBySubject.test,
            subject: props.testBySubject.subject,
            students: dataSource,
         })
   ...
   const columns = [
      ...
      {
         title: `Score ${
            test.marksObtainable
               ? String(test.marksObtainable) + "/" + String(100)
               : ``
         }`,
         dataIndex: "score",
         key: "score",
            ...
            return (
               <div style={{ textAlign: "center" }}>
                  <Input
                     ...
                     min={0}
                     max={test.marksObtainable ? test.marksObtainable : 0}
                     style={{
                        background:
                           x !== 0 || c.hasScore ? "white" : "#f5222dcc",
                     }}
                     value={x}
                  />
               </div>
   ...
   const getStudentTestScore = (value, tests) => {
      ...
      value.subjectName = props.subject.subjects.find(
         (x) => x._id === value.subjectId
      ).name;
      return props.getStudentTestScore(value).then((x) => {
   ...
   return (
      <>
         <Card
            title={`${
               props.testBySubject.subject && props.testBySubject.test
                  ? `Add score for ${capitalize(
                       props.testBySubject.subject.name
                          ? props.testBySubject.subject.name
                          : ""
                    )}  ${capitalize(
                       props.testBySubject.test.name
                          ? props.testBySubject.test.name
                          : ""
                    )} `
                  ...
                  <Table
                     ...
                     footer={() => (
                        <Popconfirm
                           ...
                           onConfirm={handleScoreSheetSubmit}
                           okText="Yes"
                           cancelText="No"
                        >
                           <Button type="primary">
...
export const getServerSideProps = wrapper.getServerSideProps(async (ctx) => {
   ...
   await store.dispatch(getStaffSubjects(staffId));
