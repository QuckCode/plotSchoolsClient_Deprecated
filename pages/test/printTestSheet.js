...
import { Card,Row, Typography,  Menu, Dropdown, Table,Avatar, Input, Button, Popconfirm, Modal, Icon, Divider } from 'antd';
...
import { capitalize, termTextToNUmbers, nth, printPDF, printPDFMultiple } from '../../lib/helpers';
...
const PrintTestSheetPage = (props) =>{
  ...
  const handlePrint = ()=>{
   if (scoreSheet.length===0)  return printPDF(`scoreSheet1`)
    return printPDFMultiple("scoreSheet")
  }
  const handleGetScoreSheetAll= (value)=>{
    axios.post(`${url}/result/caSheet/all/subject`, value)
    .then(({data})=>{
      setScoreSheet(data)
  ...
  const handleGetScoreSheetSingle= (value)=>{
    axios.post(`${url}/result/caSheet/single/subject`,value )
    .then(({data})=>{
       setScoreSheet(data)
       setHiddenTable(false)
    })
  ...
  return (
    <>
      <Card 
        ...
        className="mb-4"> 
          <div className="p-2">
              { !hiddenTable? ( <Button style={{margin:5}} type="primary"  icon="print" onClick={()=>handlePrint(true)}>{ scoreSheet.length>1? " Print All Sheet Ca Sheet":" Print Ca Sheet"}</Button>) : (<></>) } 
              {
                !hiddenTable ?
                  (
                  scoreSheet.map((data,i )=>(
                    <div id={`scoreSheet${i}`} className="scoreSheet">
                     <br/>
                     <br/>
                      <Table title={ ()=>(
                         <div style={{ width:"100%", textAlign:"center"}}>
                             <Typography.Title  level={4} strong style={{padding:10, margin:0}} > 
                               {capitalize(data.school.name)}
                             </Typography.Title>
                             <Divider style={{padding:0, margin:0}}/>
                             <Typography.Title level={4} strong  style={{padding:10, margin:0}} > 
                                  {`${termTextToNUmbers(data.school.term)+nth(termTextToNUmbers(data.school.term))} Term ${data.school.section} Continuos Assessment Sheet`}
                             </Typography.Title>
                             <Divider style={{padding:0, margin:0}}/>
                             <Typography.Title level={4} strong  style={{padding:10, margin:0}} >
                      ...
                      } bordered pagination={false}  size='large' dataSource={data.students}  columns={columns} scroll={true} />
                    </div>
                ...
                :(
                  <PrintScoreSheetForm 
                     ...
                     arms={props.arm.arms}  subjects= {props.subject.subjects}/>
                )
             }
             { !hiddenTable? ( <Button style={{margin:10}} icon="arrow-left" onClick={()=>setHiddenTable(true)}> Back To form</Button>) : (<></>) } 
          </div>
       </Card>
    </>
  )
