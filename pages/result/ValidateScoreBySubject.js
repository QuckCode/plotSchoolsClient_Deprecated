...
const ValidateScoreBySubjectPage = ({
   classes,
   sections,
   arms,
   showResult,
   ...
   const handleSubmit = (value) => {
      setLoading(true);
      Router.push({
         pathname: "/result/ValidateScoreBySubject",
         query: { arm: value.arm, class: value.classN, subject: value.subject },
      }).then(() => {
         setLoading(false);
      });
   };
   const handleGoBack = () => {
      Router.push({ pathname: "/result/ValidateScoreBySubject" }).then(() => {
         setLoading(false);
      });
   };
            ...
            <div className="p-4">
               {currentClassTests.length == 0 ? (
                  <Result
                     ...
                     extra={
                        <Button
                           onClick={goToClassSetting}
                           type="primary"
                           key="console"
                        >
                           {" "}
                           Click here to go to class tests settings{" "}
                        </Button>
                     }
                  />
               ) : (
                  <div>
                     <div>
                        <span style={{ textDecoration: "underline" }}>
                           {" "}
                           Class: {
                              classes.find((x) => x._id === classN).name
                           }{" "}
                           {arms.find((x) => x.id === arm).arm}{" "}
                        </span>
                     </div>
                     <br />
                     <div>
                        ...
                        <span style={{ marginLeft: "10rem" }}>
                           ...
                           Subject :{" "}
                           {capitalize(
                              subjects.find((x) => x._id === subject).name
                           )}{" "}
                        </span>
                     </div>
                     <br />
                     ...
                     <Row gutter={[16, 16]}>
                        <Col>
                           <Table
                              ...
                              columns={parseClassTestToTable(currentClassTests)}
                           />
                        </Col>
                     </Row>
                     <Button
   ...
   try {
      ...
      if (ctx.query.arm && ctx.query.class) {
         try {
            ...
            await store.dispatch(
               getStudentValidatedScoreRequest({
