...
const getPreviousResult = async (term,section,admissionNumber) =>{
  try {
    let data =  await  (await Axios.post(`${url}/result/previous`, {term:term, section:section, admissionNumber:admissionNumber})).data
    return data
  } catch (error) {
     return []
  }
...
const PrintPreviousResultPage = ({showResult, schoolSettings={}, results=[], tests=[]}) => {
 ...
 const parseResultColumn =  (data)=>{
  let c=  []
  c.push({title:"Position",key:"position", dataIndex:"position",  render: text => <span>{ text+nth(text)} </span>,})
  c.push({title:"Class",
  ...
  let v =  data.reverse().map(({name, marksObtainable})=>({title:`${name+" ("+marksObtainable+")%"}`,key:name, dataIndex:"scores" , 
 ...
 const parseAttendanceColumn= ()=>{
  ...
  if(!showResult){
    return (
      ...
      className="mb-4"> 
        <div className="p-5">
             <PreviousResultForm submit={handleSubmit} previousSections={schoolSettings.previousSchoolSection}/>
        </div>
      </Card>
    )
  }
      ...
      <div id="result" className="p-4">
           ...
           <Row className="rowForm">
            ...
            <div style={{textAlign:"start"}}>
                <span> Name of student :  {results.Student[0].firstName +" "+results.Student[0].middleName+" "+results.Student[0].srnName} </span>
            </div>
            ...
            <Col span={12}>
              <div style={{textAlign:"end"}}>
                <Avatar shape="square"  size={100}  src={results.Student[0].passport}/>
              </div>
            </Col>
           </Row>
           <Row>
          ...
          <Row gutter={[16, 16]}>
              <Col span={8}>
                  <Table size="small" dataSource={results.resultBehaviour[0].behaviourScores} pagination={false} bordered columns={parseColumn('behaviour')}/>
              </Col>
            <Col span={8}>
                <Table size="small"  dataSource={results.resultSkill[0].skillScores} pagination={false} bordered columns={parseColumn("skill")}/>
            </Col>
            <Col  span={8}>
...
export const getServerSideProps = wrapper.getServerSideProps(
  async (ctx ) => {
    ...
    if(!ctx.query.term && !ctx.query.section && !ctx.query.admissionNumber)
        return {props:{ showResult:false, schoolSettings:propStore.schools.settings }}
    else{
      let results = await getPreviousResult(ctx.query.term,ctx.query.section,ctx.query.admissionNumber)
      return {props:{ showResult:true, schoolSettings:propStore.schools.settings, results:results, tests:propStore.test.tests }}
    }
  } catch (error) {
       redirectError(ctx)
  }
