...
const PrintResultPage = ({showResult,classes, sections,  arms, currentClassTests=[], results, schoolSettings={}}) => {
  ...
  const  handleSubmit= (value)=>{
     setLoading(true)
      ...
      .then(data=>{
         Router.push({ pathname:`/result/PrintResult`, query:{classN:value.classN, arm:value.arm, school:school}})
        setLoading(false)
      })
      .catch(({response})=>{
        setLoading(false)
    ...
    return (
      ...
      className="mb-4">
       <div id="result" className="p-4">
          ...
          <Button onClick={printAll} type="primary" style={{marginLeft:"1rem"}} > Prints All Student Result  </Button>
      </Card>
    )
  }
}
