...
const ClassesSubjectsPage = (props) => {
   ...
   const loadSubject = (value) => {
      props
         ...
         .then(() => {
            setClassName(
               props.classes.classes.find((x) => x._id === value.class).name
            );
            setClassId(
               props.classes.classes.find((x) => x._id === value.class)._id
            );
            props
               .getCurrentClassSubjects(
                  props.classes.classes.find((x) => x._id === value.class)._id
   ...
   const addClassSubject = (e) => {
      ...
      return setAddSubjectList(
         addSubjectList.filter((x) => x !== e.target.value)
      );
   };
   ...
   const removeClassSubject = (e) => {
      ...
      return setRemoveSubjectList(
         removeSubjectList.filter((x) => x !== e.target.value)
      );
   };

   const saveAddedSubject = () => {
      props
         .addSubjects({ subjects: addSubjectList, classId: classId })
         .then(() => {
            Modal.success({
   ...
   const saveRemovedSubject = () => {
      props
         .removeSubject({ subjects: removeSubjectList, classId: classId })
         .then(() => {
            Modal.success({
               title: "Updated subject",
            });
   ...
   const handleOnSave = () => {
      ...
      setClassId("");
      setClassName(""), setAddSubjectList([]);
                     ...
                     >
                        <List
                           ...
                           footer={
                              <div>
                                 <Button
                                    onClick={saveAddedSubject}
                                    disabled={addSubjectList.length == 0}
                                    type="primary"
                                 >
                                    {" "}
                                    Add Subjects{" "}
                     ...
                     >
                        <List
                           ...
                           footer={
                              <div>
                                 <Button
                                    onClick={saveRemovedSubject}
                                    disabled={removeSubjectList.length == 0}
                                    type="danger"
                                 >
                                    {" "}
                                    Remove Subjects{" "}
...
export default PrivateRoute(
   connect(mapStateToProps, mapDispatchToProps)(ClassesSubjectsPage)
);
